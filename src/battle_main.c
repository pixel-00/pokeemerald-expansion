...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(SHINY_HUNTER, "SHINY HUNTER"),
    ...
    TRAINER_CLASS(FACTORY_HEAD, "{PKMN} TRAINER"),
            ...
            if (!(gBattleControllerExecFlags & ((gBitTable[battler]) | (0xF << 28) | (gBitTable[battler] << 4) | (gBitTable[battler] << 8) | (gBitTable[battler] << 12))))
            {
                ...
                case B_ACTION_SWITCH:
                    *(gBattleStruct->battlerPartyIndexes + battler) = gBattlerPartyIndexes[battler];
                    ...
                    else if ((ItemId_GetHoldEffect(gBattleMons[battler].item) != HOLD_EFFECT_SHED_SHELL && gBattleMons[battler].ability != ABILITY_RUN_AWAY)
